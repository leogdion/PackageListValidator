...
public struct Validator {
  ...
  static func verifyPackageDump(at directoryURL: URL, _ callback: @escaping ((PackageError?) -> Void)) {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + Validator.processTimeout) {
  ...
  static func verifyPackage(at gitURL: URL, withSession session: URLSession, _ callback: @escaping ((PackageError?) -> Void)) {
    let processSemaphore = DispatchSemaphore(value: Validator.semaphoreCount)
    ...
    _ = download(packageSwiftURL, withSession: session) { result in
      ...
      Validator.self.verifyPackageDump(at: outputDirURL) {
        error in
  ...
  static func filterRepos(_ packageUrls: [URL], withSession session: URLSession, includingMaster: Bool, _ completion: @escaping ((Result<[URL], Error>) -> Void)) {
    ...
    session.dataTask(with: Validator.masterPackageList) { data, _, error in
      ...
      do {
        allPackageURLs = try Validator.decoder.decode([URL].self, from: data)
      } catch {
        completion(.failure(error))
        return
      }
    ...
    for (index, gitURL) in packageUrls.enumerated() {
      ...
      concurrentQueue.async {
        ...
        self.verifyPackage(at: gitURL, withSession: session) {
          ...
          if Validator.displayProgress && Validator.logEveryCount < packageUnsetResults.count {
            DispatchQueue.main.async {
              count += 1
              if count % (packageUnsetResults.count / Validator.logEveryCount) == 0 {
                print(".", terminator: "")
              }
            }
          } else if error == nil {
